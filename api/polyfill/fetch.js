...
dope.initComponent({
        ...
        win.Headers = class Headers {
            constructor (init, isImmutable) {
                ...
                if (init) {
                    ...
                    } else if (init instanceof win.Headers) {
    ...
    polyfillRequest (mkKey, win) {
        ...
        win.Request = class Request {
            constructor (input, init) {
                ...
                if (init) {
                    defIf("method", "GET");
                    def("headers", new win.Headers(init.headers));
                    if (init.body) {
    ...
    polyfillResponse (dope, mkKey, win) {
        var keyBody = mkKey("body");
        win.Response = class Response {
            constructor (body, init) {
                ...
                Object.def(this, {
                    ...
                    headers: {
                        value: new win.Headers(opts.headers, true),
                        writable: false,
                        configurable: false,
                        enumerable: true
                    },
    ...
    init (dope) {
        "use strict";
        const win = window;
        const mkKey = win.Symbol ? (name => window.Symbol(name)) : (name => `___${name}`);
        const ciEq = (a, b) => 0 === a.localeCompare(b, "en", {
            sensitivity: "accent"
        });
        if (!win.Headers) {
            this.polyfillHeaders(dope, mkKey, ciEq, win);
        }
        if (!win.Request) {
            this.polyfillRequest(mkKey, win);
        }
        if (!win.Response) {
            this.polyfillResponse(dope, mkKey, win);
        }
            ...
            if (input instanceof win.Request) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    xhr.onreadystatechange = function () {
                        if (4 === xhr.readyState) {
                            try {
                                resolve(new win.Response(xhr.response, {
                                    status: xhr.status,
                                    statusText: xhr.statusText,
                                    headers: new win.Headers(xhr.getAllResponseHeaders(), true),
                                    url: request.url
                                }));
                            } catch (e) {
                                reject(e);

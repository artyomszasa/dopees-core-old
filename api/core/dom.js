...
(function () {
    const undef = dope.undefined,
        ...
        doc = document,
        regexWS = /\s+/,
        pPolyfill = dope.component('core.polyfill'),
        pWeakMap = pPolyfill.then(() => dope.polyfill('WeakMap')),
        todos = [pPolyfill.then(() => dope.polyfill('Element.classList'))];
    ...
    // component only resolved when pWeakMap has been loaded....
    let userData;
    todos.push(pWeakMap.then(() => userData = new WeakMap()));
    ...
    dope.async(dopeVars.component, Promise.all(todos));
}());